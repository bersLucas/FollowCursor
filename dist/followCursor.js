...
var followCursor = function followCursor(elemList) {
  ...
  $elems.reduce(function(total, img) {
    ...
    img.onmousemove = function(e) {
      ...
      //get key
      var count = this.getAttribute("data-mouserotate");

      //Cursor's X position in relation to the element (0-1)
      var xPos = (e.clientX - boxSizes[count].left) / boxSizes[count].width;

      //Cursor's Y position in relation to the element (0-1)
      var yPos = (scrollPos + e.clientY - boxSizes[count].top) / boxSizes[count].height;

      //Rotation calculation
      var rotateY = scale * -1 + xPos * (scale * 2);
      var rotateX = scale - yPos * scale * 2;
...
var transformCSS = function transformCSS(perspective, rotateX, rotateY) {
  var css = "transform: perspective(" + perspective + "px) rotateY(" + rotateY + "deg) rotateX(" + rotateX + "deg);";
